...
- [ ] `releases.go`：release 列表、版本比较、按 GOOS/GOARCH 选资产、`komari-SHA256SUMS` 解析
- [ ] `download.go`：流式下载 + SHA256 校验（含"校验和不符"与"资产缺失"两条失败路径）
- [ ] `install.go`：前置检查（容器/systemd/可写）、`--version` 自检、backup + 原子替换
- [ ] `state.go`：状态文件读写（重启后仍可读）
- [ ] 单测：`internal/upgrade/*_test.go`
...
- [ ] `web/rpc/jsonrpc/admin.upgrade.go`：`admin:listServerReleases` / `admin:upgradeServer` / `admin:upgradeStatus`
- [ ] 仅管理员（沿用 `/api/admin` 权限组）；重复触发返回 `already running`
- [ ] 审计日志：发起人、from→to、结果（走 `utils/log`，不新建业务表）
- [ ] 设置项 `update_repo`（默认 `zhemed/komari`）与开关 `upgrade_enabled`（默认开）
- 验证：`go test ./web/rpc/jsonrpc/...`；手工 `curl -X POST /api/rpc2`（未登录应为 403）

## Step 3 · 发版流程配套（必须先于 E2E）

- [ ] 构建脚本产出 `komari-SHA256SUMS`（两行：amd64/arm64）
- [ ] `docs/MAINTAINING.md` §3.4 第 6 步：资产清单/命令加它（17 → 18 个资产）
- [ ] 说明"≤0.0.7 的 release 无此资产 → 不支持一键升级"
- 验证：本地构建后 `sha256sum -c dist/komari-SHA256SUMS` 通过

## Step 4 · 前端

- [ ] `AdminPanelBar.tsx`：弹窗内按钮组（立即升级 / 安装此版本 / 复制 pull 命令）
- [ ] 状态机：`upgradeStatus` 轮询 + 断连即"重启中" + `/api/version` 轮询至版本变化（60s 超时）
- [ ] 失败展示：错误原文 + 手工恢复命令
- [ ] i18n 5 语言（zh_CN/zh_TW/en/ja/id）
- [ ] `./scripts/build-frontend.sh` 重新构建 → 更新 `scripts/frontend-build.env` 的 `FRONTEND_TREE_SHA256`
...
- [ ] `docs/MAINTAINING.md`：新增"面板一键升级"章节（支持矩阵、失败恢复、安全边界）
- [ ] `README.md`：功能点一句（含"容器请 pull 镜像"）
- [ ] `.trellis/spec/backend/` 新增 `server-upgrade.md`（RPC 契约、状态机、不变量、测试要求）
