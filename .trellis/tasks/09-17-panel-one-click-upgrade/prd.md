...
- [x] 面板驱动的版本切换实测通过：0.0.9 → 0.0.7、0.0.9 → 0.0.8 均成功，切换后
      `sha256sum /opt/komari/komari` 与对应 release 资产**逐字节一致**
      （**注**：原验收写的"0.0.7 用面板升到 0.0.8"不可实现——0.0.7 里没有本功能；
      "升到最新"分支在有更高版本时的点击路径见 implement.md"未覆盖"一节）
- [x] 数据不丢：两次升级往返后累计流量单调不减（3259→3541→3549 MB），rollups 行数不减
- [x] 回滚实测：0.0.9 → 0.0.7 与 0.0.9 → 0.0.8 各有一次成功记录，并留下 `komari.backup.<旧版本>`
- [x] 校验失败/自检失败一律不替换：单测覆盖（校验和不符、自检版本行不符），
      且 0.0.8 那次真实失败（执行位缺陷）实测**没有**动线上二进制与数据
- [x] 权限与开关：清空 api_key 后旧凭据被拒（Permission denied）；
      开关关闭时 `enabled=false` 且升级请求返回 PermissionDenied（-32041）
- [x] 容器分支由单测覆盖（只给 pull 命令、`Execute` 不做任何替换）；**未**在真实容器内跑过（如实标注）
- [x] `komari-SHA256SUMS` 已进 release（资产数 17→18），安装脚本实测打印"校验和匹配"，
      自升级按它校验；顺带给 0.0.7 的 release 补了该资产（按当时发布的二进制计算）
- [x] `./scripts/check-repo.sh --full` 十项全绿；新增单测见 `internal/upgrade/*_test.go` 与
      `web/rpc/jsonrpc/admin.upgrade_test.go`
- [x] 实测留证：`logs` 表审计记录、`upgrade-state.json`、备份文件、
      `.build/e2e_upgrade.py` 的输出（含版本/哈希/数据对比）
