# 维护本仓库（komari 自维护版 · 当前 0.0.15）

本仓库是由 **zhemed 独立维护的 komari 发行版**：版本线从 **0.0.1** 起步（当前 **0.0.15**），
...
| 组件 | 固定值 | 说明 |
|---|---|---|
| 项目版本 | `0.0.15`（唯一默认值在 `scripts/version.env`） | 构建时由 `scripts/build-komari.sh` 以 ldflags 注入 `CurrentVersion`；agent 用同一版本号 |
